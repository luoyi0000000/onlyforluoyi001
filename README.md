...
`config/tools.ts` 里现在只保留“转换节点”和“探针”两个常用外链。以后添加自己的站点时，继续在同一个配置文件中追加条目即可。

站点仍保留可编辑的 Hero、五个 JSX 插槽、主题令牌和工具详情页外壳，方便继续扩展。
...
```ts
{
  ...
  iconSrc: '/images/my-tool.png',                   // 可选：public/ 下的自定义卡片图标
